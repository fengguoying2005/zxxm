...
public class CzysmssendListALO extends BaseALO {

	protected void doService() {
		PageBean pageBean = reqEnv.getData("pageBean");
		DBPersistenceManager pm = this.getPM();
		StringBuffer sql = new StringBuffer("SELECT T.NSRDATA_LSH AS HID_LSH,T.* FROM SMS_NSRDATA T WHERE SMSTYPE_DM='05' ");
		...
		List<Czysmssend> orgsmssends = null;
		if(pageBean!=null) {
			orgsmssends = pm.queryPageList(sql.toString() , Czysmssend.class, args , pageBean);
		} else {
			orgsmssends = pm.queryForList(sql.toString() , Czysmssend.class, args);
		}
		try {
