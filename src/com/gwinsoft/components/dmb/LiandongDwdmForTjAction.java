...
public class LiandongDwdmForTjAction extends BaseAction<BaseIAO> {
	...
	public String getDWDM() throws Exception {
		...
		try {
			pm = DBHelper.getPm();
			int N = pm.queryForInt("SELECT SUM(SL) AS SL FROM T_KC WHERE HP_DM='"+dm+"'");
			kv.setValue2(N+"");
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
		...
		map = DMB.getDMB("HPZL_CACHE", "JQJ_JE");
