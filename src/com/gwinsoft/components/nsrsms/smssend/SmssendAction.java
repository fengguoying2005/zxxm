...
public class SmssendAction extends BaseAction<BaseIAO> {
		private Group group;
		...
		public String listmx() {
			String SMSZT_DM = request.getParameter("SMSZT_DM");
			...
			this.putRequestData("SMSZT_DM", SMSZT_DM);
			this.putRequestData("MODTYPE", MODTYPE);
			...
			if ("01".equals(SMSZT_DM) || "1".equals(MODTYPE)) {
				...
				for(NSRDATAMX msg : msgs) {
					...
					jsonObject.put("SKSSQ_Q", CommonTool.format10(msg.getSKSSQ_Q()));
					jsonObject.put("SKSSQ_Z", CommonTool.format10(msg.getSKSSQ_Z()));
				...
				for(Nsrmsg msg : msgs) {
					...
					jsonObject.put("FS_SJ", msg.getFS_SJ());
		...
		public String see() {
			this.putRequestData("FIND_SMSZT_DM", FIND_SMSZT_DM);
			this.putRequestData("NSRDATA_LSH", NSRDATA_LSH);
		...
		public Map getDxmbmap() {
			...
			while(it.hasNext()) {
				...
				if(user.getORG_DM_JG().equals(cache.get(key).get("ORG_DM_JG"))) {
					if(group.getSMSTYPE_DM().equals(cache.get(key).get("SMSTYPE_DM"))) {
