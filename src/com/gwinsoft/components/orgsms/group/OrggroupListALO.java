...
public class OrggroupListALO extends BaseALO {

	protected void doService() {
		PageBean pageBean = reqEnv.getData("pageBean");
		DBPersistenceManager pm = this.getPM();
		StringBuffer sql = new StringBuffer("SELECT T.GROUP_LSH AS HID_LSH,T.* FROM ORG_GROUP T WHERE 1=1 ");
		...
		List<Orggroup> orggroups = null;
		if(pageBean!=null) {
			orggroups = pm.queryPageList(sql.toString() , Orggroup.class, args , pageBean);
		} else {
			orggroups = pm.queryForList(sql.toString() , Orggroup.class, args);
		}
		try {
