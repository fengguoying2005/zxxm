...
public class OrggroupModALO extends BaseALO {

	protected void doService() {
		String message = "";
		User user = reqEnv.getData("user");
		Orggroup orggroup = reqEnv.getData("orggroup");
		...
		//pm.executeUpdate("DELETE FROM ORG_GROUPMX WHERE GROUP_LSH='"+BH+"'");
		String sql = "";
		...
		for(GROUPMX mx:tablemx) {
			...
			sql = "INSERT INTO ORG_GROUPMX(GROUPMX_LSH,GROUP_LSH,ORG_DM_JG,ORG_DM_BM,USER_DM,SJHM,LRRY_DM,XGRY_DM,LR_SJ,XG_SJ,ORG_MC_JG,ORG_MC_BM,USER_MC,ZW_DM,ZW_MC) " +
...
//		}
		sql = "delete from ORG_GROUPMX where GROUPMX_LSH in ("+
		"    select temp.GROUPMX_LSH from ("+
		"        select a.GROUPMX_LSH  from ORG_GROUPMX a  where a.GROUP_LSH='"+BH+"'"+
		"        and exists ("+
		"            select 1 from ORG_GROUPMX b where a.GROUP_LSH=b.GROUP_LSH and a.USER_DM=b.USER_DM group by b.GROUP_LSH,b.USER_DM having count(b.USER_DM) > 1"+
		"        )"+
		"        and not exists ("+
		"            select * from (select min(c.GROUPMX_LSH) GROUPMX_LSH from ORG_GROUPMX c where c.GROUP_LSH='"+BH+"' group by c.GROUP_LSH,c.USER_DM having count(c.USER_DM)>1) d where d.GROUPMX_LSH=a.GROUPMX_LSH"+
		"        )"+
		"    ) temp"+
		")";
		pm.executeUpdate(sql);
		...
		//select count
		sql = "SELECT COUNT(0) FROM ORG_GROUPMX WHERE GROUP_LSH='"+BH+"'";
		int nn = pm.queryForInt(sql);
		orggroup.setGROUP_COUNT(nn+"");
		pm.executeUpdate("UPDATE ORG_GROUP SET ORG_DM_JG=?,GROUP_NAME=?,GROUP_COUNT=?,XG_SJ=?,XGRY_DM=? WHERE GROUP_LSH=?", orggroup);
		message="修改成功！";
