...
public class DxmbAction extends BaseAction<BaseIAO> {
		private Dxmb dxmb;
		private List<Dxmb> dxmbs;
		...
		public String toAdd() {
			...
			dxmb.setORG_DM_JG(UserHelper.getLoginUserFromRequest(request).getORG_DM_JG());
			return "add";
		}
		public String toMod() {
			see();
		...
		public String add() {
			dxmb.setORG_DM_JG(UserHelper.getLoginUserFromRequest(request).getORG_DM_JG());
			String msg = SmsUtil.validate(dxmb.getINFO());
	...
	public IComplexPrintData getExcelPrintData() {
		...
		int i = 0;
		for (Dxmb _dxmb: dxmbs) {
			i ++;
			list.add(new CommonExcelPrintBean().setObject(""+i)
					.setObject(_dxmb.getORG_DM_JG())
					.setObject(_dxmb.getSMSTYPE_DM())
					.setObject(_dxmb.getDXMB_MC())
					.setObject(_dxmb.getINFO())
					.setObject(_dxmb.getBZ())
					.setObject(_dxmb.getLRRY_DM())
					.setObject(_dxmb.getLR_SJ()));
		}
		...
		public void setDxmb(Dxmb dxmb) {
				this.dxmb=dxmb;
		}
		public Dxmb getDxmb(){
				return this.dxmb;
		}
		public void setDxmbs(List<Dxmb> dxmbs) {
				this.dxmbs=dxmbs;
		}
		public List<Dxmb> getDxmbs(){
				return this.dxmbs;
		}
		...
		public Map getDxysmap() {
			return DMB.sortByKey(DMB.getDMB("DXYS_CACHE", "DXYS_MC"),true);
		}
