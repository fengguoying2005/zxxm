...
public class JKTX implements InterruptableJob {
	...
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		...
		String from = JobUtils.getFsJobStart("JKTX");
		String to = JobUtils.getZjkDate();
		System.out.println("缴款成功提醒,"+from+"->"+to);
		String sql = "SELECT A.*,B.NSRMC,B.ORG_DM AS SWJGBM,B.FRSJH,B.CWJLSJH,B.BSRYSJH,SYSDATE AS FS_SJ FROM (select to_char(XTSPHM) as XTSPHM,JKLX_DM,NSRSBM,JK_RQ,SKSSQ,SKSSZ,SUM(SF_JE) AS SF_JE,count(0) AS CNT,lob_to_char(WMSYS.WM_CONCAT(ZSXM_DM)) AS ZSXM_DMS,lob_to_char(WMSYS.WM_CONCAT(ZSPM_DM)) AS ZSPM_DMS,lob_to_char(WMSYS.WM_CONCAT(SF_JE)) AS SF_JES,MAX(A.LR_SJ) AS LR_SJ from T_SMS_JKTX A where A.LR_SJ>=TO_DATE('"+from+"','yyyy-mm-dd hh24:mi:ss') and A.LR_SJ<TO_DATE('"+to+"','yyyy-mm-dd hh24:mi:ss') group by XTSPHM, JKLX_DM, NSRSBM, JK_RQ, SKSSQ, SKSSZ HAVING COUNT(0) < 20) A,NSR_JBXX B WHERE A.NSRSBM = B.NSRBM";
		String sql_insert = "insert into DX_JKTX(LSH,XTSPHM,JKLX_DM,NSRSBM,JK_RQ,SKSSQ,SKSSZ,SF_JE,CNT,ZSXM_DMS,ZSPM_DMS,SF_JES,NSRMC,SWJGBM,SJHM,SJLX,SMSINFO,LR_SJ,BZ,FSZT_DM,FS_SJ) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		DBPersistenceManager pm = null;
		try {
			pm = DBHelper.getPm();
			List<JKTX_BEAN> beans = pm.queryForList(sql, JKTX_BEAN.class);
			for(JKTX_BEAN bean : beans) {
				try {
					bean.setFSZT_DM("06");
					bean.setBZ(bean.getXTSPHM());
					if("ALL".equals(JSRLX_DM)) {
						bean.setLSH("JKFR"+JobUtils.getLSH());
						bean.setSJLX("FR");
						...
						bean.setLSH("JKCW"+JobUtils.getLSH());
						bean.setSJLX("CWJL");
						...
						bean.setLSH("JKBS"+JobUtils.getLSH());
						bean.setSJLX("BSRY");
						msg = DxmbUtil.genSMS(bean, INFO);
						bean.setSMSINFO(msg);
						pm.executeUpdate(sql_insert, bean);
