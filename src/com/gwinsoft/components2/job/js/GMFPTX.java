...
//		System.out.println("购买发票提醒状态");
		String type_dm = "GMFPTX";
		...
		try {
			String sql = "SELECT LSH FROM DX_GMFPTX WHERE FS_SJ>=TO_DATE('"+from+"','YYYY-MM-DD HH24:MI:SS') and FS_SJ<TO_DATE('"+to+"','YYYY-MM-DD HH24:MI:SS')";
			pm = DBHelper.getPm();
		...
		for(String LSH : LSHS) {
			ScheduleTool.schedule(new GMFPTX_T(LSH));
		}
	}
...
class GMFPTX_T extends Thread {
	private String LSH;
	public GMFPTX_T(String LSH) {
		this.LSH = LSH;
	}
	public void run() {
		try {
			...
			DBPersistenceManager pm = null;
			try {
				String sql = "UPDATE DX_GMFPTX SET FSZT_DM='"+zt+"' WHERE LSH='"+LSH+"'";
				pm = DBHelper.getPm();
