...
//		System.out.println("缴款提醒状态");
		...
		try {
			String sql = "SELECT LSH FROM DX_JKTX WHERE FS_SJ>=TO_DATE('"+from+"','YYYY-MM-DD HH24:MI:SS') and FS_SJ<TO_DATE('"+to+"','YYYY-MM-DD HH24:MI:SS')";
			pm = DBHelper.getPm();
		...
		for(String LSH : LSHS) {
			ScheduleTool.schedule(new JKTX_T(LSH));
		}
	}
...
class JKTX_T extends Thread {
	private String LSH;
	public JKTX_T(String LSH) {
		this.LSH = LSH;
	}
	public void run() {
		try {
			...
			DBPersistenceManager pm = null;
			try {
				String sql = "UPDATE DX_JKTX SET FSZT_DM='"+zt+"' WHERE LSH='"+LSH+"'";
				pm = DBHelper.getPm();
