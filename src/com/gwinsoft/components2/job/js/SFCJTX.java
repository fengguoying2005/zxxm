...
//		System.out.println("催缴提醒状态");
		String type_dm = "SFCJTX";
		...
		try {
			String sql = "SELECT LSH FROM DX_SFCJTX WHERE FS_SJ>=TO_DATE('"+from+"','YYYY-MM-DD HH24:MI:SS') and FS_SJ<TO_DATE('"+to+"','YYYY-MM-DD HH24:MI:SS')";
			pm = DBHelper.getPm();
		...
		for(String LSH : LSHS) {
			ScheduleTool.schedule(new SFCJTX_T(LSH));
		}
	}
...
class SFCJTX_T extends Thread {
	private String LSH;
	public SFCJTX_T(String LSH) {
		this.LSH = LSH;
	}
	public void run() {
		try {
			...
			DBPersistenceManager pm = null;
			try {
				String sql = "UPDATE DX_SFCJTX SET FSZT_DM='"+zt+"' WHERE LSH='"+LSH+"'";
				pm = DBHelper.getPm();
