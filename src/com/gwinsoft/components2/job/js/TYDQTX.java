...
//		System.out.println("停业到期提醒状态");
		String type_dm = "TYDQTX";
		...
		try {
			String sql = "SELECT LSH FROM DX_TYDQTX WHERE FS_SJ>=TO_DATE('"+from+"','YYYY-MM-DD HH24:MI:SS') and FS_SJ<TO_DATE('"+to+"','YYYY-MM-DD HH24:MI:SS')";
			pm = DBHelper.getPm();
		...
		for(String LSH : LSHS) {
			ScheduleTool.schedule(new TYDQTX_T(LSH));
		}
	}
...
class TYDQTX_T extends Thread {
	private String LSH;
	public TYDQTX_T(String LSH) {
		this.LSH = LSH;
	}
	public void run() {
		try {
			...
			DBPersistenceManager pm = null;
			try {
				String sql = "UPDATE DX_TYDQTX SET FSZT_DM='"+zt+"' WHERE LSH='"+LSH+"'";
				pm = DBHelper.getPm();
