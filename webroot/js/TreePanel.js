...
TreeNode.prototype={
	initEl : function(){
		...
		this['html-element']['emp']	 = document.createElement('img');
		this['html-element']['emp'].src = this.getOwnerTree().getIcon('empty');
		...
		this['html-element']['element'].appendChild(this['html-element']['emp']);
	...
	render : function(){
		...
		if(this.isRoot()){
			var child = this['html-element']['element'];
			//add by fenggy 2010-10-06
			child.removeChild(this['html-element']['emp']);
			this.ownerTree.element.appendChild(child);
			this.expand();
		}else{
			var child = this['html-element']['element'];
			this.parentNode['html-element']['child'].appendChild(child);
		}
		this.paintPrefix();
	...
	paintLine : function(){
		...
		for(var i = 1 ,count = pathNodes.length-1 ; i < count ; i++){
				...
				//	img2.src = ownerTree.getIcon('empty');
				//	this['html-element']['line'].appendChild(img2);
				//} else {
					//alert("node.isRoot()"+node.attributes["text"]);
				//}
		}
	},
	paintClipIcoImg : function(){
		...
		this['html-element']['clip'].style.cursor='hand';
	},
	paintIconImg : function(){
		...
		this['html-element']['icon'].style.cursor='hand';
	},
