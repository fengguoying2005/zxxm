...
function Datepicker() {
	this._defaults = {
		...
		triggerImage: '',//new add
		triggerAlt:'',//new add
		triggerTitle:'',//new add
...
$.extend(Datepicker.prototype, {
	...
	defaultRenderer: {
		...
		'<table class="dateTable"><thead>{weekHeader}</thead><tbody>{weeks}</tbody></table></div>',
	...
	_attachments: function(target, inst) {
		...
		var triggerImage = inst.get('triggerImage');
		var triggerAlt = inst.get('triggerAlt');
		var triggerTitle = inst.get('triggerTitle');
		inst.trigger = (!trigger ? $([]) :
			//$(trigger).clone().removeAttr('id')
			$('<img/>').attr({ src: triggerImage, alt: triggerAlt, title: triggerTitle,style:"cursor:hand" }).addClass(this._triggerClass)
				[inst.get('isRTL') ? 'insertBefore' : 'insertAfter'](target).
				click(function() {
					if (!$.datepick.isDisabled(target[0])) {
						$.datepick[$.datepick.curInst == inst ? 'hide' : 'show'](target[0]);
					}
				}
			)
		);
