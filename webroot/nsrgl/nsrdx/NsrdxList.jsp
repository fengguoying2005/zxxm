...
<script type="text/javascript">
            function next(event) {
                ...
                if("del"==event) {
                	if("初始"!=$(":radio:checked").parent().parent().children('td').eq(4).html().trim()
              			  &&"审核不通过"!=$(":radio:checked").parent().parent().children('td').eq(4).html().trim()) {
                  	  alert("只有‘初始’或‘审核不通过’状态下才能进行删除。");
                  	  return;
              	  	}
                ...
                document.forms[0].action="<%=basePath%>nsrgl/nsrdx_"+event+".html";
...
<body onload="tablecloth('nsrdxList');">
<form id="myform" name="myform" action="nsrdx" method="post">
...
<table class="record-list" id="nsrdxList" name="nsrdxList">
	<thead>
		<tr>
			<th>选择</th>
			<th>行</th>
	...
	<tfoot>
		<tr>
			...
			审核人<input id="SHY" name="SHY" type="text" class="easyui-combobox" data-options="editable:false,valueField:'DM',textField:'MC',url:'<%=basePath%>ajax/usertree!shycombox.action'">
				<input id="shBtn" class="buttonface" type="button" value="提交审核" onClick="next('SH')" />
				<input class="buttonface" type="button" value="编辑短信" onClick="next('toAdd')" />
				<input id="delBtn" class="buttonface" type="button" value="删除" onClick="next('del')" />
				...
				<input id="expBtn" class="buttonface" type="button" value="导出" onClick="next('printExcel')" />
			</div>
			</td>
		</tr>
	</tfoot>
	<tbody>
		<s:iterator value="nsrdxs" status="stat">
			<tr
				...
				<td>
				<s:property value="%{getText('{0,date,yyyy-MM-dd HH:mm:ss}',{LR_SJ})}" /></td>
