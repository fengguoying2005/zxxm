...
<script type="text/javascript">
		...
		function deleterows() {
			...
			for(var i=0; i<rows.length; i++){
			    ids.push(rows[i].SFXCMSG_LSH.split(",")[0]);
			}
			if(ids.length==0) {
				...
				$.messager.confirm('确认', '是否确认删除纳税人?', function(r){
					if (r){
						var keyids = ids.join(',');
						$.post("<%=basePath%>"+'nsrgl/nsrdx!deletemx.action?KEYIDS='+keyids+'&LSH='+$("#NSRDATA_LSH").val(),
							function(data) {
		...
		<table id="datagrid" title='纳税人明细信息' iconCls='icon-save' fitColumns=true singleSelect=false fit=true toolbar="#tb">  
		    <thead>  
		        <tr>
					...
					<th data-options="field:'MSG',width:100,formatter:msgformat" editor='text'>短信内容</th>
					<!-- 
