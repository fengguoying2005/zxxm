            ...
            function next(event) {
                ...
                if("toAdd"!=event && "list"!=event && "printExcel"!=event &&"printWord"!=event) {
                    ...
                    if(len>0){
                      ...
                      if(event=="del") {
                    	  if("初始"!=$(":radio:checked").parent().parent().children('td').eq(5).html().trim()
                    			  &&"已生成短信"!=$(":radio:checked").parent().parent().children('td').eq(5).html().trim()
                    			  &&"审核不通过"!=$(":radio:checked").parent().parent().children('td').eq(5).html().trim()) {
                        	  alert("只有‘初始’、‘已生成短信’或‘审核不通过’状态下才能进行删除。");
                        	  return;
                    	  }
                      } else if(event=="toMod") {
                    	  if("初始"!=$(":radio:checked").parent().parent().children('td').eq(5).html().trim()
                    			&&"已生成短信"!=$(":radio:checked").parent().parent().children('td').eq(5).html().trim()
                      			&& "审核不通过"!=$(":radio:checked").parent().parent().children('td').eq(5).html().trim()) {
                        	  alert("只有‘初始’、‘已生成短信、‘审核不通过’状态才能修改。");
                        	  return;
                    	  }
                      } else if(event=="send") {
                    	  if("已生成短信"!=$(":radio:checked").parent().parent().children('td').eq(5).html().trim()
                    			  &&"审核不通过"!=$(":radio:checked").parent().parent().children('td').eq(5).html().trim()) {
                        	  alert("只有在‘已生成短信’或‘审核不通过’状态下才能提交审核。");
                        	  return;
                    	  }
                ...
                if("del"==event) {
                    ...
                    if(!window.confirm("确定发送短信吗？请注意：点击确定后，短信将由审核人员进行审核后短信才能发出。")) {
                        return false;
                    }
                }
                document.forms[0].action="<%=basePath%>nsrsms/smssend_"+event+".html";
	...
	<tfoot>
		<tr>
			...
			<div align="right">
				...
				value="提交审核" onClick="next('send')" /> 
				<input class="buttonface"
				type="button" value="删除短信" onClick="next('del')" /> 
				<input
				class="buttonface" type="button" value="短信生成"
				onClick="next('toMod')" /> 
				<input class="buttonface" type="button"
				value="查看" onClick="next('see')" /> 
				<input class="buttonface"
				type="button" value="导出" onClick="next('printExcel')" /></div>
			</td>
		</tr>
	</tfoot>
	<tbody>
